...
OVPNConfig::OVPNConfig(QObject *parent)
    : QObject(parent)
    , m_bSuccess(false)
...
void OVPNConfig::get(const QString &username, const QString &password)
{
    m_username = username;
    m_password = password;
    m_bSuccess = false;
...
bool OVPNConfig::generate(const QString &server, PROTOCOL_OPENVPN protocol, int port)
{
    ...
    if (m_tempFile.open())
    {
        ...
        const QString upScript = qApp->applicationDirPath() + "/../Resources/tap-up-down.sh";
        const QString downScript = qApp->applicationDirPath() + "/../Resources/tap-up-down.sh";
...
void OVPNConfig::onReplyFinished(QNetworkReply* reply)
{
    reply->deleteLater();
